...
def _resolve_prep_source_earth_args(args) -> dict:
    ...
    if missing:
        raise ValueError(
            "prep-source-earth requires the following settings either via --config or CLI flags: "
            + ", ".join(missing)
        )

    resolved["bandpass"] = _normalize_pair(resolved.get("bandpass"), "bandpass")
    resolved["time_window"] = _normalize_pair(resolved.get("time_window"), "time-window")
    resolved["downsample_hz"] = _normalize_optional_float(
        resolved.get("downsample_hz"),
        "downsample-hz",
    )
    resolved["plot_summary"] = _normalize_optional_bool(
        resolved.get("plot_summary"),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    prep_source_parser = subparsers.add_parser(
        "prep-source-earth",
        help="Prepare source-array Earth SAC files into TAPIR CSV inputs",
    )
    prep_source_parser.add_argument(
        "--config",
        default=None,
        help="Optional YAML file for source-array Earth preprocessing settings",
    )
    prep_source_parser.add_argument("--data-dir", default=None, help="Directory containing input SAC files")
    prep_source_parser.add_argument("--output-dir", default=None, help="Directory to write TAPIR-ready CSV files")
    prep_source_parser.add_argument(
        "--noise-dir",
        default=None,
        help="Optional directory containing matching source-array noise SAC files",
    )
    prep_source_parser.add_argument(
        "--bandpass",
        type=lambda value: _parse_pair(value, "bandpass"),
